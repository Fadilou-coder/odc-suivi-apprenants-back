    ...
    public void run(String... args) throws Exception {
        ...
        for (int i=0;i<competenceList.toArray().length;i++){
            niveauEvaluationRepository.saveAll(Arrays.asList(
                    new NiveauEvaluation("Niveau 1", "D’autres attentes sont exprimées dans les offres d’emploi par les recruteurs. Ainsi, parmi les qualités ", "Concevoir et développer un site : conception, modélisation et architecture d’applications, méthodes, normes, langages et outils de développement", competenceList.get(i), referentielRepository.findByLibelle("DEVELOPPEUR DATA").get()),
